<?
	...
	class connection{
		...
		public static function connect(){
			try {
				$config = array(
					'typeDB' => 'mysql', # driver DB
					"host" => 'localhost', # HOST
					"dbname" => 'name_db', # DB name
					"user" => 'user_db',  # DB user
					"pass" => 'password_db' # DB password
				);
				# config
	...
	class DB extends connection{	
		...
		public static function getDataWhereIN($name_table, $data, $f='', $n=''){
			$orderBy = (!empty($f)) ? self::orderBy($f, $n) : '';
			if (self::checkArray($data)){
				if (empty($name_table)) return 'Error: empty name table';  
				$sh = self::$DBH->prepare("SELECT * FROM `$name_table` WHERE `". key($data) ."` IN (". rtrim(str_repeat('?,', count($data)), ',') .") $orderBy");
				$sh->execute(array_values($data));
				$sh = $sh->fetchAll();
				return (count($sh) > 1) ? $sh : array_shift($sh);
			} else {
		...
		public static function deleteFromWhereIN($name_table, $data){
			if (empty($fields)) return 'Error: empty fields'; 
			if (self::checkArray($data)){
				return (!empty($name_table)) ? self::$DBH->prepare("DELETE FROM `$name_table` WHERE `". key($data) ."` IN (". rtrim(str_repeat('?,', count($data)), ',') .") ")->execute(array_values($data)) : 'Error: empty name table';
			} else {
				return 'Error: empty data';
			}
		}# end delete From IN
